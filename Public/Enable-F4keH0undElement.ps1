<#
.SYNOPSIS
    Re-enables deployed Windows artifact deception elements.

.DESCRIPTION
    Marks Windows artifact elements as armed on target hosts and records
...
#>
function Enable-F4keH0undElement {
    ...
    process {
        foreach ($currentElementId in @($ElementId)) {
            ...
            if ([string]$currentState.Status -eq 'Removed') {
                Write-Warning "[WARNING] ElementId '$currentElementId' is removed and cannot be enabled."
                continue
            }
            ...
            $action = "Enable Windows deceptive element '$currentElementId'"
            if ($PSCmdlet.ShouldProcess($target, $action)) {
                try {
                    ...
                    Write-Warning "[WARNING] Failed enabling element '$currentElementId' on '$resolvedComputer'. Error: $($_.Exception.Message)"
                }
            }
        ...
        return [PSCustomObject]@{
            EnabledCount = $updatedRecords.Count
            Platform     = 'Windows'
        }
    }
}
